...
執行查詢後，會回傳一個 [DMarginResponse]({{ "/API/daccount#ddata.get_margin" | relative_url }}) 物件，其中包含查詢的結果。

如果 ok == True，表示查詢成功，回傳的保證金資料可以從  data 中取得回傳保證金資料,其欄位請參考 [DMargin]({{ "/API/daccount#ddata.DMargin" | relative_url }}) 。 
